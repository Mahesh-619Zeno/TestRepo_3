...
class RecurringTaskTemplate:
    ...
    def __init__(self, template_id, base_task_info, frequency, recurrence_end_date=None, last_generated_at=None, active=True):
        self.template_id = template_id  # unique identifier
        self.base_task_info = base_task_info  # dict with title, description, priority
        self.frequency = frequency  # string from allowed frequencies
        self.recurrence_end_date = recurrence_end_date  # optional ISO date string or None
        self.last_generated_at = last_generated_at or datetime.utcnow().isoformat()
        self.active = active  # to allow pausing
...
class RecurringTaskManager:
    ...
    def generate_tasks(self):
        """Check each active template and generate new task instances if due."""
        now = datetime.utcnow()
        created_tasks = []
        for template in self.templates:
            ...
            # Only generate if next_date is now or past and before end date (if specified)
            if next_date <= now and (end_date is None or next_date <= end_date):
                base = template.base_task_info
                # Create new Task object with base details
